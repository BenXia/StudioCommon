...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = @"提示";
        NSString *message = [NSString stringWithFormat:@"不允许选择超过%zd张", self.maximumImagesCount];
        ...
        if(assetRep != nil) {
            if (_returnsImage) {
                ...
                if (img) {
                    ...
                    [workingDictionary setObject:[[UIImage alloc] init] forKey:UIImagePickerControllerOriginalImage];
                }
            }

            [workingDictionary setObject:[(NSObject *)[asset valueForProperty:ALAssetPropertyURLs] valueForKey:[[[asset valueForProperty:ALAssetPropertyURLs] allKeys] objectAtIndex:0]] forKey:UIImagePickerControllerReferenceURL];
