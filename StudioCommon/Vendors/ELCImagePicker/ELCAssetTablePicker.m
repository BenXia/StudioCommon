...
- (void)viewDidLoad
{
    ...
    if (self.immediateReturn) {
        ...
        [self.navigationItem setTitle:@"努力加载中..."];
    }
...
- (void)preparePhotos
{
    @autoreleasepool {
        ...
        NSMutableArray *elcAssetsToSet = [[NSMutableArray alloc] init];
        [self.assetGroup enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            if (!isAssetFiltered) {
                [elcAssetsToSet addObject:elcAsset];
            }

        }];

        dispatch_sync(dispatch_get_main_queue(), ^{
            self.elcAssets = elcAssetsToSet;
            [self.tableView reloadData];
            // scroll to bottom
            ...
            [self.navigationItem setTitle:self.singleSelection ? @"选择单张": @"选择多张"];
        });
    }
}
...
{
	return [ELCAssetCell cellHeight];
}
