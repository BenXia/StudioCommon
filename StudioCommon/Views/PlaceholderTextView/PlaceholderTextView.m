...
- (void)drawRect:(CGRect)rect
{
    if([[self placeholder] length] > 0) {
        if (_placeHolderLabel == nil) {
            CGFloat heightToSet = [self.placeholder pp_boundFrameHeightWithMaxSize:CGSizeMake(self.bounds.size.width - 16, 9999)
                                                                           andFont:self.font];
            
            _placeHolderLabel = [[UILabel alloc] initWithFrame:CGRectMake(8, 8, self.bounds.size.width - 16, heightToSet)];
